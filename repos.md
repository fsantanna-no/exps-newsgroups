...
alt.test             ← hier 2 (repo-1-only)
alt.test.flood       ← hier 2 (repo-1-only)
```

### Repo-2's 5 newsgroups

```
comp.lang.lua        ← standalone (shared)
sci.crypt            ← hier 1 (shared)
sci.crypt.random     ← hier 1 (shared)
rec.music            ← hier 2 (repo-2-only)
rec.music.synth      ← hier 2 (repo-2-only)
```
...
repo-2/
  comp.lang.lua/
    (empty — will receive from repo-1 via sync)
  sci.crypt/
    (empty)
  sci.crypt.random/
    (empty)
...
### Repo-2 messages (5 messages, 4 authors)
...
```bash
mkdir -p ~/exps-newsgroups-lab/repo-1
cd ~/exps-newsgroups-lab/repo-1
git init
...
# Write all repo-1 messages (see section 5) into the directories
...
### Step 6b: Create repo-2

```bash
mkdir -p ~/exps-newsgroups-lab/repo-2
cd ~/exps-newsgroups-lab/repo-2
git init

# Create repo-2-only directories with messages
mkdir -p rec.music rec.music.synth

# Write all repo-2 messages (see section 5) into the directories
# ... (each message written as the numbered file)
...
```bash
# In repo-1, add repo-2 as a remote
cd ~/exps-newsgroups-lab/repo-1
git remote add peer-2 file://$(realpath ../repo-2)

# In repo-2, add repo-1 as a remote
cd ~/exps-newsgroups-lab/repo-2
...
#!/bin/bash
# sync.sh <peer-name>
# Example: cd repo-2 && bash sync.sh peer-1
...
# Example: cd repo-2 && bash watch-sync.sh peer-1 5
...
### Step 8b: Add repo-2 as a mailbox

1. Go to **File → Add mailbox → MH...**
2. Enter the path: `~/exps-newsgroups-lab/repo-2`
3. Click OK
...
### Step 9a: Start the sync watcher on repo-2
...
Then on repo-1, pull from repo-2:
...
▼ Lua 5.5 coroutine changes         alice   (11, root)
  ├─ Re: Lua 5.5 coroutine changes  carol   (21, from repo-1)
  └─ Re: Lua 5.5 coroutine changes  eve     (12, from repo-2)
